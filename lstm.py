...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        attention_weight = torch.softmax(torch.matmul(query, key.transpose(1, 2)), dim=-1)
        
        for i in range(attention_weight.shape[-1]):
            if i == 0:
                #print(attention_weight.shape, encoder_outputs.shape)
                c_prev = attention_weight[:, :, i] * encoder_outputs[:, i, :]
            else:
                c_prev += attention_weight[:, :, i] * encoder_outputs[:, i, :]
        ...
        return log_softmax, hidden, attention_weight
